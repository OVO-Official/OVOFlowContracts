 ...
 * @returns {Promise<*>}
 * */
export const deployNyatheesOVO = async () => {
	const OVOAdmin = await getOVOAdminAddress();
	await mintFlow(OVOAdmin, "10.0");

	await deployContractByName({ to: OVOAdmin, name: "NonFungibleToken" });

	const addressMap = { NonFungibleToken: OVOAdmin };
	return deployContractByName({ to: OVOAdmin, name: "NyatheesOVO", addressMap });
};
 ...
 * @returns {Promise<*>}
 * */
export const setupNyatheesOVOOnAccount = async (account) => {
...
export const getNftItem = async (account, tokenId) => {
	const name = "nft/get_nft_item";
	const args = [account, tokenId];
